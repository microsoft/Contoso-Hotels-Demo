...
function Format-ErrorMessage($description, $err) {
    $message = $description
    $message += "`nMessage: $($err.Exception.Message)"
    $message += "`nLine number: $($err.InvocationInfo.ScriptLineNumber)"
    $message
}
...
if ($RecoveryPlanContext.FailoverDirection -ne "PrimaryToSecondary") {
    ...
    Try {
        ...
        $SubscriptionContext = Get-AzSubscription -SubscriptionId $Conn.SubscriptionID  | Write-Verbose
    }
    Catch {
        Write-Error -Message (Format-ErrorMessage 'Login to Azure subscription failed.' $_) -ErrorAction Stop
    }
    Try {
        ...
        $LoadBalancer = Get-AzLoadBalancer -Name $LBName -ResourceGroupName $LBRgName
    }
    Catch {
        Write-Error -Message (Format-ErrorMessage 'Failed to retrieve Load Balancer info from Automation variables.' $_) -ErrorAction Stop
    }
    ...
    Try {
        ...
        foreach ($VMID in $VMinfo) {
            ...
            $ARMNic = Get-AzResource -ResourceId $AzureVm.NetworkProfile.NetworkInterfaces[0].Id
            $Nic = Get-AzNetworkInterface -Name $ARMNic.Name -ResourceGroupName $ARMNic.ResourceGroupName
    ...
    Catch {
        Write-Error -Message (Format-ErrorMessage 'Failed to associate the VM with the Load Balancer.' $_) -ErrorAction Stop
    }
}